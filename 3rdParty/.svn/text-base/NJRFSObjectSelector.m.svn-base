...
{
	BOOL usedSetClass = NO;
	BOOL usedDecodeClassName = NO;
	
	if([decoder respondsToSelector: @selector(setClass:forClassName:)])
	{
		usedSetClass = YES;
		[(NSKeyedUnarchiver *) decoder setClass: [NSButtonCell class] forClassName: @"NSButtonCell"];
	}
	else if([decoder respondsToSelector: @selector(decodeClassName:asClassName:)])
	{
		usedDecodeClassName = YES;
		[(NSUnarchiver *) decoder decodeClassName: @"NSButtonCell" asClassName: @"DKButtonCell"];
	}
	...
	else if(usedDecodeClassName)
	{
		[(NSUnarchiver *) decoder decodeClassName: @"NSButtonCell" asClassName: @"NSButtonCell"];
	}
